...
$Realizados = mysqli_query($con, "SELECT*FROM encuesta_usuario WHERE Id_usuario='$Id_user'"); 
while ($row = mysqli_fetch_assoc($Realizados)) {
    $IdUsuaReul = $row['Id_usuario'];
    $IdEncuResul = $row['Id_encuesta'];
}
?>
...
<body>
    ...
    <?php else : ?>
        ...
        <div>  
            ...
            <?php if($Id_user == $IdUsuaReul) : ?>
                <?php
                $EncuReali = mysqli_query($con, "SELECT*FROM encuestas WHERE Id_encuesta='$IdEncuResul'"); 
                    while($row = mysqli_fetch_assoc($EncuReali)) {?>
                <a class="link text" href="#"><?php echo $row['Titulo']; ?></a>
                <?php } ?>
                <?php else : ?>
